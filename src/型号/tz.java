...
/**
 * @author Administrator 2048小游戏
 * 
 */
public class tz extends JApplet {
	private static final int UP = 0, DOWN = 1, LEFT = 2, RIGHT = 3;
	private static final int WID = 150, SP = 10;
	private int scores = 0;
	private static JLabel scoreLabel;
	private boolean change, CheckMode = false, gameOverFlag = false,
			successFlag = false;
	private int[] label = { 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048 };
	private Color[] clo = { new Color(200, 200, 200), new Color(228, 228, 160),
	...
	class My2048Panel extends JPanel {
		private int[] xindex = { SP, 2 * SP + WID, 3 * SP + 2 * WID,
				4 * SP + 3 * WID };
		private int[] yindex = { SP, 2 * SP + WID, 3 * SP + 2 * WID,
				4 * SP + 3 * WID };
		...
		public void paintComponent(Graphics g) {
			...
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 4; j++) {
					if (rset[i][j] != null) {
						g.setColor(Color.WHITE);
						g.drawRoundRect(yindex[j], xindex[i], WID, WID,
								WID / 5, WID / 5);
						...
						g.fillRoundRect(yindex[j], xindex[i], WID, WID,
								WID / 5, WID / 5);
	...
	class GameOverPane extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			int width = fm.stringWidth("游戏失败");
			int height = fm.getHeight();
			g.setColor(new Color(255, 0, 0));
			g.drawString("游戏失败!", getWidth() / 2 - width / 2, getHeight()
					/ 2 - height / 2);
		}
	}
	...
	class SuccessPane extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			g.setFont(font);
			FontMetrics fm = Toolkit.getDefaultToolkit().getFontMetrics(font);
			int width = fm.stringWidth("游戏成功!");
			int height = fm.getHeight();
			g.setColor(new Color(255, 0, 0));
			g.drawString("游戏成功!", getWidth() / 2 - width / 2, getHeight()
					/ 2 - height / 2);
		}
	}
	...
	class LOGO extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			int height = fm.getHeight();
			g.setColor(new Color(255, 0, 0));
			g.drawString("4366小游戏", 20, getHeight() / 2 + 20);
		}
	}
	...
	public class goBackListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			// for (int i = 0; i < 4; i++) {
			// for (int j = 0; j < 4; j++) {
			// int num = Integer.parseInt(String.valueOf(arr.get(4 * i + j
			// + 1)));
			// if (num != 0) {
	...
	public void gameOver() {
		...
		final JButton jb1 = new JButton("Again");
	...
	public static void run(JApplet applet, int x, int y, int width, int height) {
